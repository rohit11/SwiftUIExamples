...
public extension HKWorkout {
    ...
    func getWorkoutDuration() -> String {
        return self.duration.stringFromTimeInterval()
    }
    
    func getWorkoutSourceName() -> String {
        return self.sourceRevision.source.name
    }
    
    func getWorkoutUIID() -> String {
        return self.uuid.uuidString
    }
    
    func caloriesBurned() -> String {
        if let caloriesBurned = self.totalEnergyBurned?.doubleValue(for: HKUnit.kilocalorie()) {
            return String(format: "CaloriesBurned: %.2f", caloriesBurned)
        }
